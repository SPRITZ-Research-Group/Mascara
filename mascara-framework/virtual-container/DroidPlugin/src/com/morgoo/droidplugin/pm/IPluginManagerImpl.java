    ...
    public ActivityInfo getActivityInfo(ComponentName className, int flags) throws RemoteException {
        ...
        try {
            ...
            if (pkg != null) {
                ...
                if (parser != null) {
                    ActivityInfo result = parser.getActivityInfo(className, flags);
                    return result;
                }
            }
        } catch (Exception e) {
...
//        Log.e(TAG, "dexFile=%s,1=%s,2=%s", dexFile, DexFile.isDexOptNeeded(apkfile), DexFile.isDexOptNeeded(PluginDirHelper.getPluginDalvikCacheFile(mContext, parser.getPackageName())));
    ...
    public ActivityInfo selectStubActivityInfo(ActivityInfo pluginInfo) throws RemoteException {
        ActivityInfo result = mActivityManagerService.selectStubActivityInfo(Binder.getCallingPid(), Binder.getCallingUid(), pluginInfo);
        return result;
    }

    @Override
    public ActivityInfo selectStubActivityInfoByIntent(Intent intent) throws RemoteException {
        Log.d("PlgnMgrI", "selectStubActivityInfoByIntent(" + intent.toString() + ")");
        ActivityInfo ai = null;
        if (intent.getComponent() != null) {
            ai = getActivityInfo(intent.getComponent(), 0);
            // Log.d("PlgnMgrI", "selectStubActivityInfoByIntent: intent.getComponent()=" + intent.getComponent().toString());
        } else {
            ResolveInfo resolveInfo = resolveIntent(intent, intent.resolveTypeIfNeeded(mContext.getContentResolver()), 0);
            // Log.d("PlgnMgrI", "selectStubActivityInfoByIntent: resolveInfo=" + resolveInfo.toString());
        ...
        if (ai != null) {
            // Log.d("PlgnMgrI", "selectStubActivityInfoByIntent: ai=" + ai.toString());
            ActivityInfo result = selectStubActivityInfo(ai);
            Log.d("PlgnMgrI", "selectStubActivityInfoByIntent returns " + result.toString());
            return result;
        }
        Log.d("PlgnMgrI", "selectStubActivityInfoByIntent returns null");
        return null;
    }
